...
Proof.
  intros n m p.
  induction p as [| p' IHp'].
    - rewrite -> mul_comm. rewrite -> mult_0_l. 
      rewrite -> mul_comm. rewrite -> mult_0_l.
      rewrite -> mul_comm. rewrite -> mult_0_l.
      simpl. reflexivity.
    - rewrite <- mult_n_Sm.
      assert (HC: n * S p' = p' * n + n). {
        rewrite <- mult_n_Sm. rewrite -> mul_comm. reflexivity.
      }
      rewrite -> HC.
      assert (HC2: m * S p' = p' * m + m). {
        rewrite <- mult_n_Sm. rewrite -> mul_comm. reflexivity.
      }
      rewrite -> HC2.
      rewrite -> plus_rearrange.
      rewrite -> IHp'.
      simpl. rewrite -> mul_comm. rewrite -> add_shuffle3.
      assert (HC3: n + p' * n + (p' * m + m) = n + (p' * n + m * p' + m)). {
        simpl. rewrite -> add_assoc. rewrite -> add_assoc. rewrite -> add_assoc.
        assert (HC3': p' * m = m * p'). {
          rewrite -> mul_comm. reflexivity.
        }
        rewrite -> HC3'. reflexivity.
      }
      simpl.
      rewrite -> HC3.
      reflexivity.
...
Proof.
  ...
  induction p as [| p' IHp'].
    - rewrite -> mul_0_r. rewrite -> mult_0_l.
      rewrite -> mul_comm. rewrite -> mult_0_l. reflexivity.
...
Proof.
  ...
  - reflexivity.  (* main goal *)
  - rewrite -> add_comm.  (* replace *)
    reflexivity.
Qed.
(** [] *)
